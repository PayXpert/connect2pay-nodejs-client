...
class GatewayClient extends PaymentClient {
     ...
     * @returns {Promise<any>}
     */
    async creditFundTransfer (requestBody) {
        if ('cardSecurityCode' in requestBody) {
            delete requestBody.cardSecurityCode;
        }

        return await this.makeRequest(config.GATEWAY_HOST, "/transaction/credit/creditcard", "POST", requestBody);
    }
